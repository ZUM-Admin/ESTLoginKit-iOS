...
> **네이버는 인앱 브라우저 경로에서 항상 재인증(`auth_type=reauthenticate`)을 요구합니다.**
> 인앱 브라우저(`ASWebAuthenticationSession`)는 Safari와 쿠키를 공유하고 네이버 SDK가 이를 끌 수
> 없게 막아둬서, 재인증이 없으면 앱에서 로그아웃해도 남아있는 네이버 세션으로 자동 통과해
> **다른 계정으로 로그인할 수가 없습니다.** 재인증은 로그인 화면만 다시 띄우고 앱 연동은
> 유지하므로 이미 동의한 항목을 다시 묻지 않습니다. 네이버앱이 설치돼 있으면 네이버앱 경로가
> 우선이라 이 설정이 적용되지 않으며, 그때는 네이버앱에 로그인된 계정을 따라가고 계정 전환도
> 네이버앱에서 합니다.
