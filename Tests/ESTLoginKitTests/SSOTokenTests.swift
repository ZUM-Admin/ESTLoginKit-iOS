...
struct SSOLoginURLTests {
    ...
    func redirectURLWithQuery() {
        let redirect = "/webview/verification?client_id=4870426&callbackURL=https://x.com/cb"
    ...
    func encodesRedirectURLOnce() {
        let redirect = "/webview/verification?client_id=8941192&callbackURL=https://test.estoneid.com/auth/app-callback"
        let url = ESTLoginManager.ssoLoginURL(redirectURL: redirect, ssoToken: "sso")

        let encoded = url.absoluteString.components(separatedBy: "redirect_url=").last
        #expect(encoded == "%2Fwebview%2Fverification%3Fclient_id%3D8941192%26callbackURL%3Dhttps%3A%2F%2Ftest.estoneid.com%2Fauth%2Fapp-callback")
    }

    @Test("redirect_url 값 추출 — path와 query 유지")
    func redirectURLValueFromURL() {
        let url = URL(string: "https://test.estoneid.com/webview/verification?client_id=1")!
        #expect(ESTLoginManager.redirectURLValue(from: url) == "/webview/verification?client_id=1")
    }
